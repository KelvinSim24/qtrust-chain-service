...
const DerivationScheme_1 = require("./DerivationScheme");
...
class Crypto {
     ...
     */
    static toMobileKey = (password, privateKey) => {
        // Errors
        ...
        // Result
        return {
            encrypted: Convert_1.Convert.uint8ArrayToHex(iv) + encrypted.ciphertext,
            salt: salt.toString(),
        };
    };
    /**
     ...
     */
    static derivePassSha = (password, count) => {
        // Errors
        if (!password) {
            throw new Error('Missing argument !');
        }
     ...
     */
    static passwordToPrivateKey = (common, walletAccount, algo) => {
        // Errors
     ...
     */
    static randomKey() {
        return (0, utils_1.randomBytes)(32);
    }
    /**
     ...
     */
    static encodePrivateKey = (privateKey, password) => {
        // Errors
        ...
        return {
            ...
            iv: Convert_1.Convert.uint8ArrayToHex(r.iv),
        };
    };
    /***
     ...
     */
    static _encode = (senderPriv, recipientPub, msg, iv, salt, dScheme = DerivationScheme_1.DerivationScheme.Ed25519Sha3) => {
        // Errors
        if (!senderPriv || !recipientPub || !msg || !iv || !salt) {
            throw new Error('Missing argument !');
        }
        // Processing
        const keyPair = KeyPair_1.KeyPair.createKeyPairFromPrivateKeyString(senderPriv, dScheme);
        const pk = Convert_1.Convert.hexToUint8(recipientPub);
        const sharedKey = KeyPair_1.KeyPair.deriveSharedKey(keyPair.privateKey, pk, salt, dScheme);
        ...
        // Result
        const result = Convert_1.Convert.uint8ArrayToHex(salt) + Convert_1.Convert.uint8ArrayToHex(iv) + CryptoJS.enc.Hex.stringify(encrypted.ciphertext);
        return result;
    };
    /**
     * Encode a message
     ...
     * @return {string} - The encoded message
     */
    static encode = (senderPriv, recipientPub, msg, dScheme = DerivationScheme_1.DerivationScheme.Ed25519Sha3) => {
        // Errors
     ...
     */
    static _decode = (recipientPrivate, senderPublic, payload, iv, salt, dScheme = DerivationScheme_1.DerivationScheme.Ed25519Sha3) => {
        // Error
        if (!recipientPrivate || !senderPublic || !payload) {
            throw new Error('Missing argument !');
        }
        // Processing
        const keyPair = KeyPair_1.KeyPair.createKeyPairFromPrivateKeyString(recipientPrivate, dScheme);
     ...
     * @return {string} - The decoded payload as hex
     */
    static decode = (recipientPrivate, senderPublic, _payload, dScheme = DerivationScheme_1.DerivationScheme.Ed25519Sha3) => {
        // Error
        ...
        return (0, utf8_1.encode)(String.fromCharCode.apply(null, Convert_1.Convert.hexToUint8(decoded)));
    };
    /**
    ...
    static randomMnemonic = (strength, rng, wordlist) => {
        const nmenomic = (0, bip39_1.generateMnemonic)(strength, rng, wordlist);
        return nmenomic;
    };
    static isValidMnemonic = (mnemonic, wordlist) => {
        return (0, bip39_1.validateMnemonic)(mnemonic, wordlist);
    };
    static mnemonicToHex = (mnemonic) => {
    ...
    static entropyToMnemonic(hex) {
        return (0, bip39_1.entropyToMnemonic)(hex);
    }
}
