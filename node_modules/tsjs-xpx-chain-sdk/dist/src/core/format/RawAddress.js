...
class RawAddress {
     ...
     */
    static stringToAddress = (encoded) => {
     ...
     */
    static aliasToRecipient = (namespaceId) => {
     ...
     */
    static addressToString = (decoded) => {
        if (RawAddress.constants.sizes.addressDecoded !== decoded.length) {
            throw Error(`${Convert_1.Convert.uint8ArrayToHex(decoded)} does not represent a valid decoded address`);
        }
     ...
     */
    static publicKeyToAddress = (publicKey, networkIdentifier) => {
        // step 1: sha3 hash of the public key
        const publicKeyHash = sha3_1.sha3_256.create().update(publicKey).digest();
        // step 2: ripemd160 hash of (1)
        const ripemdHash = ripemd160_1.ripemd160.create().update(publicKeyHash).digest();
        ...
        const hash = sha3_1.sha3_256.create().update(decodedAddress.subarray(0, RawAddress.constants.sizes.ripemd160 + 1)).digest();
    ...
    static isValidAddress = (decoded) => {
        ...
        RawArray_1.RawArray.copy(checksum, RawArray_1.RawArray.uint8View(hash.digest()), RawAddress.constants.sizes.checksum);
     ...
     */
    static isValidEncodedAddress = (encoded) => {
        if (RawAddress.constants.sizes.addressEncoded !== encoded.length) {
            return false;
        }
        try {
