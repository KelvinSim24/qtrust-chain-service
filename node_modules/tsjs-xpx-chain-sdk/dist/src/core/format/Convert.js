...
class Convert {
     ...
     * @returns {Uint8Array} A uint8 array corresponding to the input.
     */
    static hexToUint8Reverse = (input) => {
        if (0 !== input.length % 2) {
            throw Error(`hex string has unexpected size '${input.length}'`);
        }
        const output = new Uint8Array(input.length / 2);
     ...
     */
    static uint8ToUint32 = (input) => new Uint32Array(input.buffer);
    /**
     ...
     */
    static uint32ToUint8 = (input) => new Uint8Array(input.buffer);
     ...
     */
    static UInt64ToUint8Array = (input) => [
        (input[0] & 0xff) >>> 0,
        (input[0] & 0xff00) >>> 8,
        (input[0] & 0xff0000) >>> 16,
