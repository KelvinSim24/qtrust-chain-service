...
class RawUInt64 {
    static readUint32At = (bytes, i) => (bytes[i] + (bytes[i + 1] << 8) + (bytes[i + 2] << 16) + (bytes[i + 3] << 24)) >>> 0;
    /**
    ...
    static toHex = (uint64) => {
        ...
        return Convert_1.Convert.uint8ArrayToHex(uint8Array);
    };
    /**
     ...
     */
    static isZero = (uint64) => 0 === uint64[0] && 0 === uint64[1];
}
