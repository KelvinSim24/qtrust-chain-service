...
exports.VerifiableTransaction = void 0;
/*
 * Copyright 2019 NEM
 *
...
class VerifiableTransaction {
    ...
    static createTransactionHash(transactionPayload, generationHashBytes) {
        ...
        SHA3Hasher_1.SHA3Hasher.func(hash, new Uint8Array(signingBytes), 32);
        return format_1.Convert.uint8ArrayToHex(hash);
    }
    /**
     ...
     */
    signTransaction(keyPair, generationHash, dScheme = crypto_1.DerivationScheme.Ed25519Sha3) {
        ...
        const keyPairEncoded = crypto_1.KeyPair.createKeyPairFromPrivateKeyString(keyPair.privateKey, dScheme);
        const signature = Array.from(crypto_1.KeyPair.sign(keyPair, new Uint8Array(signingBytes), dScheme));
        ...
        const payload = format_1.Convert.uint8ArrayToHex(signedTransactionBuffer);
        return {
            payload,
     ...
     * @returns {module:model/TransactionPayload} - Signed Transaction Payload
     */
    static signTransactionPayload(txnPayload, keyPair, generationHash, dScheme = crypto_1.DerivationScheme.Ed25519Sha3) {
        const generationHashBytes = Array.from(format_1.Convert.hexToUint8(generationHash));
        const byteBuffer = Array.from(format_1.Convert.hexToUint8(txnPayload));
        const signingBytes = new Uint8Array(generationHashBytes.concat(byteBuffer.slice(4 + 64 + 32)));
        const keyPairEncoded = crypto_1.KeyPair.createKeyPairFromPrivateKeyString(keyPair.privateKey, dScheme);
        const signature = Array.from(crypto_1.KeyPair.sign(keyPair, new Uint8Array(signingBytes), dScheme));
        const signedTransactionBuffer = byteBuffer
        ...
        const payload = format_1.Convert.uint8ArrayToHex(uint8Data);
        return {
            payload,
            hash: VerifiableTransaction.createTransactionHash(payload, generationHashBytes),
        };
    ...
    serializeUnsignedTransaction() {
        return format_1.Convert.uint8ArrayToHex(this.serialize());
    }
    /**
     * @param {KeyPair} keyPair KeyPair instance
     * @param {DerivationScheme} dScheme The derivation scheme
     ...
     */
    newSignCosignatoriesTransaction(keyPair, dScheme = crypto_1.DerivationScheme.Ed25519Sha3) {
        const signature = crypto_1.KeyPair.sign(keyPair, new Uint8Array(this.bytes), dScheme);
        return {
            parentHash: format_1.Convert.uint8ArrayToHex(this.bytes),
            signature: format_1.Convert.uint8ArrayToHex(signature),
            scheme: format_1.Convert.uint8ToHex(dScheme),
    ...
    signCosignatoriesTransactionV1(keyPair) {
        const signature = crypto_1.KeyPair.sign(keyPair, new Uint8Array(this.bytes), crypto_1.DerivationScheme.Ed25519Sha3);
        return {
            parentHash: format_1.Convert.uint8ArrayToHex(this.bytes),
            signature: format_1.Convert.uint8ArrayToHex(signature),
            signer: keyPair.publicKey,
