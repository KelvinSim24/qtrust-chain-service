...
class AggregateTransactionService {
    ...
    isComplete(signedTransaction) {
        ...
        return (0, rxjs_1.from)(aggregateTransaction.innerTransactions).pipe((0, operators_1.mergeMap)((innerTransaction) => this.accountHttp.getMultisigAccountInfo(innerTransaction.signer.address)
            .pipe(
        /**
        ...
        (0, operators_1.mergeMap)((_) => _.minApproval !== 0 && _.minRemoval !== 0 ?
            this.accountHttp.getMultisigAccountGraphInfo(_.account.address)
                .pipe((0, operators_1.map)((graphInfo) => this.validateCosignatories(graphInfo, signers, innerTransaction))) : (0, rxjs_1.of)(signers.find((s) => s === _.account.publicKey) !== undefined)))), (0, operators_1.toArray)()).pipe((0, operators_1.mergeMap)((results) => {
     ...
     * @param cosignatures
     */
    static newAddCosignatures(signedTransaction, cosignatures) {
        ...
        if (recreatedSignedTx.type !== TransactionType_1.TransactionType.AGGREGATE_COMPLETE_V2) {
        ...
        const signedTransactionRaw = AggregateTransaction_1.AggregateTransaction.appendSignatures(signedTransaction, cosignatures.filter(cosignature => !recreatedSignedAggregateComplete.signedByAccount(model_1.PublicAccount.createFromPublicKey(cosignature.signer, recreatedSignedAggregateComplete.version.networkType))));
     ...
     * @param cosignatures
     */
    static addCosignaturesV1(signedTransaction, cosignatures) {
        // re-create the transaction from payload to determine the type - only allow aggregate complete transaction as an input
        const recreatedSignedTx = TransactionMapping_1.TransactionMapping.createFromPayload(signedTransaction.payload);
        if (recreatedSignedTx.type !== TransactionType_1.TransactionType.AGGREGATE_COMPLETE_V1) {
            throw new Error('Only serialized signed aggregate complete v1 transaction allowed.');
        }
        const recreatedSignedAggregateComplete = recreatedSignedTx;
        const signedTransactionRaw = AggregateTransaction_1.AggregateTransaction.appendSignaturesV1(signedTransaction, cosignatures.filter(cosignature => !recreatedSignedAggregateComplete.signedByAccount(model_1.PublicAccount.createFromPublicKey(cosignature.signer, recreatedSignedAggregateComplete.version.networkType))));
        return new SignedTransaction_1.SignedTransaction(signedTransactionRaw.payload, signedTransaction.hash, signedTransaction.signer, signedTransaction.type, signedTransaction.networkType);
    }
}
